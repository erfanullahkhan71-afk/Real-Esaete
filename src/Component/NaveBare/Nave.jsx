...
function Nave() {
  ...
  return (
    <nav className="fixed top-4 left-1/2 transform -translate-x-1/2 w-11/12 lg:w-4/5 bg-white/20 backdrop-blur-xl shadow-2xl rounded-3xl flex justify-between items-center px-6 py-3 z-50">
      ...
      <ul className="hidden lg:flex gap-8 font-medium text-gray-700 items-center">
        {menuItems.map((item, i) => (
          <li key={i} className="relative group cursor-pointer">
            <Link
              to={item.to}
              className="px-3 py-1 rounded-md hover:bg-white/30 transition"
            >
              {item.label}
            </Link>
      ...
      <div
        className="lg:hidden ml-4 cursor-pointer flex flex-col gap-1"
        onClick={() => setIsOpen(!isOpen)}
      >
        <span
          className={`block w-6 h-0.5 bg-gray-500 transition-all ${
      ...
      {isOpen && (
        <ul className="absolute top-full left-1/2 transform -translate-x-1/2 mt-4 w-4/5 bg-white/90 backdrop-blur-lg rounded-2xl shadow-lg flex flex-col items-center py-4 gap-4 lg:hidden">
          {menuItems.map((item, i) => (
          ...
          <li className="w-full text-center">
            <button
              ...
              className="w-full px-6 py-2 rounded-full bg-gradient-to-r from-green-400 to-blue-500 text-white font-semibold shadow-lg hover:scale-105 transition-transform duration-300"
            >
              Sign Up
            </button>
          </li>
